...
def keyword_loop(
    ...
    lexical_weight: float = typer.Option(
        2.0,
        "--lexical-weight",
        min=0.0,
        help="Score multiplier for local lexical/BM25 scoring when a lexical workflow is used.",
    ),
    retrieval_workflow: str = typer.Option(
        "semantic_constraints",
        "--retrieval-workflow",
        help="Retrieval workflow: semantic_only, semantic_constraints, lexical_only, hybrid_rrf, hybrid_rrf_constraints.",
    ),
    rrf_k: int = typer.Option(
        60,
        "--rrf-k",
        min=1,
        help="RRF k parameter for hybrid_rrf workflows.",
    ),
    llm_rerank: bool = typer.Option(
        False,
        "--llm-rerank",
        help="Use an LLM to rerank the retrieved candidates after deterministic scoring.",
    ),
    llm_rerank_top_n: int = typer.Option(
        20,
        "--llm-rerank-top-n",
        min=1,
        help="Number of deterministic candidates to send to the optional LLM reranker.",
    ),
