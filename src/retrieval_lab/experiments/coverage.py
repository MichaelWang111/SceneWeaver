...
from retrieval_lab.compat import LEGACY_COMMANDS
...
def audit_infra_coverage_command(args: Any) -> dict[str, Any]:
    ...
    summary = {
        ...
        "legacy_command_coverage": round(
            sum(1 for row in rows if row["status"] in {"native", "partial"}) / max(1, len(LEGACY_COMMANDS)),
            6,
        ),
        "core_experiment_replacement_coverage": core_experiment_replacement_coverage(rows),
        "core_experiment_compat_only_count": sum(
            1 for row in rows if row["command"] in CORE_EXPERIMENT_COMMANDS and row["status"] == "compat_only"
        ),
...
def core_experiment_replacement_coverage(command_rows: list[dict[str, Any]]) -> float:
    core_rows = [row for row in command_rows if row["command"] in CORE_EXPERIMENT_COMMANDS]
    return round(
        sum(1 for row in core_rows if row["status"] in {"native", "partial"}) / max(1, len(core_rows)),
        6,
    )
