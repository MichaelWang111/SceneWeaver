...
def qrel_has_vote_conflict(row: dict[str, Any]) -> bool:
    votes = active_adjudication_grades(row)
    return len(set(votes)) > 1


def active_adjudication_grades(row: dict[str, Any]) -> list[int]:
    vote_rows = [vote for vote in row.get("grade_votes", []) if isinstance(vote, dict)]
    for judge_type in ("human", "llm", "bootstrap"):
        typed = [vote for vote in vote_rows if qrel_vote_judge_type(vote, row) == judge_type]
        if typed:
            return [int(vote.get("grade", row.get("grade", 0))) for vote in typed]
    return []


def qrel_needs_adjudication(row: dict[str, Any]) -> bool:
    explicit_need = bool(row.get("needs_adjudication")) and qrel_is_bootstrap_only(row)
    return explicit_need or qrel_has_vote_conflict(row) or qrel_confidence(row) < 0.6
