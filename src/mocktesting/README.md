...
```powershell
python -m mocktesting.mock_retriever evaluate --limit 0 --top-k 10
```
...
```powershell
python -m mocktesting.mock_retriever evaluate --split dev --limit 0 --top-k 10
python -m mocktesting.mock_retriever evaluate --split test --limit 0 --top-k 10
python -m mocktesting.mock_retriever evaluate --split hidden --limit 0 --top-k 10
```

推荐先跑纯 embedding 的 test 基线：

```powershell
python -m mocktesting.mock_retriever evaluate --split test --limit 0 --top-k 10 --no-constraints --output src\mocktesting\eval_outputs\mock_retrieval_report_no_constraints.json
```

再只用 dev 调参，并自动输出 test 对照：

```powershell
python -m mocktesting.mock_retriever tune-constraints --split dev --top-k 10
python -m mocktesting.mock_retriever evaluate --split test --limit 0 --top-k 10
python -m mocktesting.mock_retriever evaluate --split hidden --limit 0 --top-k 10
```
...
```powershell
python -m mocktesting.mock_retriever evaluate-leave-one-fixture-out --top-k 10
```

新增报告会包含：

```text
forbidden_stage_violation_at_1 / 3 / 10
desired_stage_hit_at_1 / 3
possible_overfit
```

## 排序 Key 扩大化验证

验证当前排序 key 是否真的优于纯 embedding：

```powershell
python -m mocktesting.mock_retriever validate-ranking-keys --split test --limit 0 --top-k 10
```
...
```text
expected_prefer_vs_forbidden_accuracy
positive_vs_random_accuracy
positive_vs_wrong_stage_accuracy
low_confidence_rate
...
```powershell
python -m mocktesting.mock_retriever validate-ranking-keys --split test --limit 0 --top-k 10 --llm-sample-size 30
```

默认输出：

```text
src/mocktesting/eval_outputs/mock_ranking_key_validation_report.json
```
...
explicit：明确说要哪个脚本阶段
fuzzy：不说阶段，只说情绪/目的
style：加入“别太像广告，要像纪录片”
negative：加入“不要炫技，不要大厂味”
constraint_first：先排除产品说明/技术炫耀，再声明真正要的脚本阶段
human_value：强调有人味，技术只作为辅助
director_brief：用导演阐述口吻描述叙事功能
mixed：保留画面元素，但强调重点不是技术展示
```

只看生成的改写文本，不调用 embedding：

```powershell
python -m mocktesting.mock_retriever validate-paraphrase-stress --split all --limit 2 --case-type simple_positive --dry-run --output src\mocktesting\eval_outputs\mock_paraphrase_stress_dry_run.json
```

正式检索评测：

```powershell
python -m mocktesting.mock_retriever validate-paraphrase-stress --split test --limit 0 --case-type simple_positive --top-k 10
```

默认输出：

```text
src/mocktesting/eval_outputs/mock_paraphrase_stress_report.json
```
