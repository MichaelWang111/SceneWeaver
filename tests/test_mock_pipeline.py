...
from sceneweaver.schemas import ExperienceCard, FilmAnalysis, FilmFingerprint, SceneAnalysis, SceneFingerprint, ScenePackage, ScenesAnalysis
...
def test_mock_pipeline_writes_valid_artifacts(tmp_path):
    ...
    assert read_json(output_dir / "fingerprints" / "scene_001.json", SceneFingerprint)
    assert read_json(output_dir / "fingerprints" / "film_fingerprint.json", FilmFingerprint)

    cards = read_jsonl(output_dir / "analysis" / "experience_cards.jsonl", ExperienceCard)
    assert len(cards) == 1
    assert cards[0].card_id == "exp_000001"
    assert cards[0].fingerprint.evidence[0].source_id == "scene_001"
